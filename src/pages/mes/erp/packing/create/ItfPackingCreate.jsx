...
import * as S from "./ItfPackingCreate.styled";
import restURI from "json/restURI.json";
import BtnComponent from "components/button/BtnComponent";
import restAPI from "api/restAPI";
import ItfPackingCreateSet from "./ItfPackingCreateSet";
...
export function ItfPackingCreate(props) {
  ...
  const onDblSelect = async (e) => {
    ...
    if (e?.targetType === "cell") {
      ...
      if (targetGrid.current === "erpOrder") {
        packingGrid?.setValue(rowKey, "request_no", data.request_no);
        packingGrid?.setValue(rowKey, "corp_code", data.corp_code);
        packingGrid?.setValue(rowKey, "plce_code", data.plce_code);
        packingGrid?.setValue(rowKey, "erp_work_order_no", data.erp_work_order_no);
        packingGrid?.setValue(rowKey, "item_id", data.item_id);
        packingGrid?.setValue(rowKey, "item_cd", data.item_cd);
        packingGrid?.setValue(rowKey, "item_nm", data.item_nm);
        packingGrid?.setValue(rowKey, "item_spec", data.item_spec);
        packingGrid?.setValue(rowKey, "order_line_dept_cd", data.line_dept_cd);
        packingGrid?.setValue(rowKey, "order_line_dept_nm", data.line_dept_nm);
        packingGrid?.setValue(rowKey, "line_dept_cd", data.line_dept_cd);
        packingGrid?.setValue(rowKey, "line_dept_nm", data.line_dept_nm);
        packingGrid?.setValue(rowKey, "work_order_date", data.work_order_date);
        packingGrid?.setValue(rowKey, "work_order_qty", data.work_order_qty);
        packingGrid?.setValue(rowKey, "order_emp_nm", data.order_emp_nm);
        packingGrid?.setValue(rowKey, "work_start_date", data.work_start_date);
        packingGrid?.setValue(rowKey, "work_end_date", data.work_end_date);
        packingGrid?.setValue(rowKey, "work_order_id", data.work_order_id);
        packingGrid?.setValue(rowKey, "dept_cd", getDeptCode(data.line_dept_nm));
        packingGrid?.setValue(rowKey, "dept_nm", await getDeptName(getDeptCode(data.line_dept_nm)));
  ...
  const getDeptCode = (lineDeptNm) => {
    const startChar = lineDeptNm.indexOf("(") + 1;
    const endChar = lineDeptNm.indexOf(")");
    const lineDeptCd = lineDeptNm.substring(startChar, endChar);
    return lineDeptCd;
  };

  const getDeptName = async (deptCode) => {
    try {
      const result = await restAPI.get(restURI.department + `?dept_cd=${deptCode}`);
      return result?.data?.data?.rows[0].dept_nm;
    } catch (err) {
      setIsSnackOpen({
        ...isSnackOpen,
        open: true,
