...
function ItfOrderSet(isEditMode) {
  /** 🔸columns ❗
   * editor: false||"text"
  ...
  const columns = [
    ...
    col.date("work_order_date", "지시일자", isEditMode, C.U, true, "text"),
    col.date("work_start_date", "시작일자", isEditMode, C.U, true, "text"),
    col.date("work_end_date", "종료일자", isEditMode, C.U, true, "text"),
    col.number("work_order_qty", "지시수량", isEditMode, C.U, C.U, true, "text"),
    col.select("order_emp_cd", "지시등록자코드", isEditMode, C.U, "center", true, "text"),
    col.select("order_emp_nm", "지시등록자", isEditMode, C.U, "center", true, "text"),
