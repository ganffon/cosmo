...
function ItfMixedCreateSet() {
  ...
  const colPerformance = [
    ...
    col.select("order_line_dept_cd", "라인부서코드", true),
    ...
    col.select("dept_cd", "생산부서코드", true),
    col.select("dept_nm", "생산부서", true),
    col.select("line_dept_cd", "생산라인부서코드", true),
    col.select("line_dept_nm", "생산라인부서", true),
    col.select("worker_group_cd", "작업조코드", true),
    col.select("worker_group_nm", "작업조", true),
    col.text("lot_no", "Lot No", true),
    col.number("work_qty", "생산량", true),
    col.select("unit_cd", "단위코드", true),
    col.select("unit_nm", "단위", true),

    col.select("input_emp_cd", "I/F등록자코드", true, C.U, "center"),
  ...
  const colSummary = [
    ...
    col.date("work_input_date", "투입일자", true),

    col.select("weigh_emp_cd", "계량자코드", true, C.U, "center"),
    col.select("weigh_emp_nm", "계량자", true, C.U, "center"),
    col.date("work_weigh_date", "계량일자", true),
  ];
  const colEmployee = [col.text("work_emp_cd", "투입자코드"), col.text("work_emp_nm", "투입자")];
  ...
  const colErpOrder = [
    ...
    col.select("line_dept_cd", "라인부서코드", C.U, C.U, C.U, true, "text"),
    ...
    col.select("order_emp_cd", "지시등록자코드", C.U, C.U, "center", true, "text"),
  ...
  const colErpDept = [
    col.text("dept_cd", "생산부서코드", C.U, C.U, C.U, C.U, true, "text"),
    col.text("dept_nm", "생산부서", C.U, C.U, C.U, C.U, true, "text"),
  ];
  ...
  const colErpWorkerGroup = [
    col.text("worker_group_cd", "작업조코드", C.U, C.U, C.U, C.U, true, "text"),
    col.text("worker_group_nm", "작업조", C.U, C.U, C.U, C.U, true, "text"),
  ];
