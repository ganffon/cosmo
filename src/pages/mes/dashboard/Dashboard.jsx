...
console.log(JSON.stringify(downtime))
  const complexColumns =  getTimeHeader()
  // setResult(GetTestValAndCreateAt(strJson));
  return (
    <SplitterLayout customClassName="my-splitter-layout"
                    percentage={true}
    ...
    >
      ...
      >
        <div>
          <div style={{ position: 'absolute', top: '20%',left: '45%', transform: 'translate(-50%, -50%)',  height: '50px' }}>라인 가동 상태</div>
          <div style={{ position: 'absolute', top: '40%', left: '20%', transform: 'translate(-50%, -50%)',  height: '50px' }}>E1 Line</div>
          {e1Status && <img src={doriGreenImg} alt="Dori Green" style={{ position: 'absolute', top: '60%', left: '20%', transform: 'translate(-50%, -50%)', width: '50px', height: '50px' }}/>}
          {!e1Status && <img src={doriRedImg} alt="Dori red" style={{ position: 'absolute', top: '60%', left: '20%', transform: 'translate(-50%, -50%)', width: '50px', height: '50px' }}/>}

          <div style={{ position: 'absolute', top: '40%', left: '45%', transform: 'translate(-50%, -50%)',  height: '50px' }}>E2 Line</div>
          {e2Status && <img src={doriGreenImg} alt="Dori Green" style={{ position: 'absolute', top: '60%', left: '45%', transform: 'translate(-50%, -50%)', width: '50px', height: '50px' }}/>}
          {!e2Status && <img src={doriRedImg} alt="Dori red" style={{ position: 'absolute', top: '60%', left: '45%', transform: 'translate(-50%, -50%)', width: '50px', height: '50px' }}/>}
          
          <div style={{ position: 'absolute', top: '40%', left: '75%', transform: 'translate(-50%, -50%)',  height: '50px' }}>E3 Line</div>
          {e3Status && <img src={doriGreenImg} alt="Dori Green" style={{ position: 'absolute', top: '60%', left: '75%', transform: 'translate(-50%, -50%)', width: '50px', height: '50px' }}/>}
          {!e3Status && <img src={doriRedImg} alt="Dori red" style={{ position: 'absolute', top: '60%', left: '75%', transform: 'translate(-50%, -50%)', width: '50px', height: '50px' }}/>}
        </div>
        <Bottom>
        {responseData && <Grid columns={columnsEmp} data={responseData.data.rows[0].worker}/>}
        </Bottom>
        </SplitterLayout>
        ...
        <ToolWrap>
          ...
          {responseData && <Chart options={cOptions}  type="bar" height={200} series={responseData.data.rows[0].line_work_status.graph}/>}
          {responseData && <GridSingle header={complexColumns} columns={columnsGoal} data={responseData.data.rows[0].line_work_status.grid}/>}
        </Bottom>
      </SplitterLayout>
    </SplitterLayout>
  );
